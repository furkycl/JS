...
export default async function Page({ params }: Params) {
  ...
  const norm = (s: string) =>
    s
      ...
      .replace(/-+/g, '-');
  const slug = norm(raw);
  const mod = midModules.find((m) => norm(m.slug) === slug || norm(m.title) === slug);
  if (!mod) {
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-8 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="grid gap-2 text-left">
            {midModules.map((m) => (
              ...
              >
                <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white text-sm font-bold">
                  {midModules.indexOf(m) + 1}
                </span>
  ...
  return <ModuleDetail mod={mod} level="mid" />;
}
